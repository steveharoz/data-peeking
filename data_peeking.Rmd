...
Explanation at https://neuroneurotic.net/2016/08/25/realistic-data-peeking-isnt-as-bad-as-you-thought-its-worse/
...
# multiple rho values
rhos = seq(0,.9, .3)
# run the simulation on each rho
Pvals = lapply(rhos, SimulOptStopCorr)
...
proportionData = Pvals %>%
  ...
  summarise(proportionSignificant = mean(p < 0.05))
...
ggplot(proportionData) +
  ...
  geom_text( hjust=0, nudge_x = .03,
    ...
    aes(label=category)) +
  scale_x_continuous(limits=c(0, 1.2), breaks=rhos) +
